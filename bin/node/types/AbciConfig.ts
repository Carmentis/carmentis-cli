...
export const AbciConfigSchema = v.object({
    genesis: v.optional(
        v.object({
    ...
    snapshots: v.optional(
        v.object({
            snapshot_block_period: v.fallback(v.number(), 10_000),
            block_history_before_snapshot: v.fallback(v.number(), 0),
            max_snapshots: v.fallback(v.number(), 2),
    ...
    abci: v.object({
        ...
        max_microblocks_per_block: v.fallback(v.number(), 50),
    }),
    paths: v.object({
...
export type AbciConfig = v.InferOutput<typeof AbciConfigSchema>;