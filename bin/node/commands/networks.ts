...
export class NetworksCommand {
    ...
    register(program: commander.Command) {
        ...
        networks
            ...
            .action(async (options) => {
                await SafeCommandRunner.safeRun(() => this.importNetworks(options.source));
            });
    ...
    private async importNetworks(source?: string) {
        ...
        try {
            ...
            const remoteNetworks: NetworksFile = await response.json();
